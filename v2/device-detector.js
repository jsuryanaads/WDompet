(function(){
  ...
  // WDompet V3.0.8 — detector + two presentation modes.
  function detectMode(){
    ...
    try { hasFinePointer = window.matchMedia('(pointer: fine)').matches; hasCoarsePointer = window.matchMedia('(pointer: coarse)').matches; } catch(e) {}
    if (width > 0) {
  ...
  function installPresentationCSS(){
    if (document.getElementById('wdompet-presentation-v308')) return;
    var style = document.createElement('style');
    style.id = 'wdompet-presentation-v308';
    style.textContent = '\
...
@media(max-width:560px){html.ui-mobile body{padding-left:78px!important}html.ui-mobile .appbar{width:calc(100% - 78px)!important;padding:8px!important}html.ui-mobile .nav{width:78px!important;padding-left:5px!important;padding-right:5px!important}.ui-mobile .nav-item{min-height:46px!important;padding:8px 3px!important}.ui-mobile .nav-item small{font-size:7.5px!important}.ui-mobile .appbar .brand-logo{width:42px!important;height:42px!important}.ui-mobile .appbar .brand b{font-size:15px!important}}';
  ...
  function updateVersion(){
    var version = 'V3.0.8';
    document.title = document.title.replace(/V3\.0\.[1-7]/g, version);
    var nodes = document.querySelectorAll('.version, footer');
    for (var i=0;i<nodes.length;i++) nodes[i].textContent = nodes[i].textContent.replace(/V3\.0\.[1-7]/g, version);
  }
  function applyMode(){
    var mode = detectMode(), root = document.documentElement;
    root.classList.remove('ui-desktop','ui-mobile'); root.classList.add(mode==='mobile'?'ui-mobile':'ui-desktop');
    root.dataset.uiMode=mode; root.dataset.uiDetector='v3.0.8'; installPresentationCSS(); updateVersion();
  }
  applyMode();
  ...
  if(window.matchMedia){try{window.matchMedia('(pointer: fine)').addEventListener('change',applyMode)}catch(e){} try{window.matchMedia('(pointer: coarse)').addEventListener('change',applyMode)}catch(e){}}
  window.wdompetUIDetector={version:'3.0.8',getMode:detectMode,apply:applyMode};
})();
